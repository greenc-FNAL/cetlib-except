...
namespace {
  ...
  template <template <typename> class COMP> class ensure_comp {
public:
    ensure_comp(std::string const & cmp) : cmp_(cmp) { }

    template <typename X>
    bool operator () (X const & left, X const & right) const
      {
        if (! COMP<X>()(left, right)) {
          std::ostringstream msg;
          msg << "FAILED comparison: "
              << left
              << ' '
              << cmp_
              << ' '
...
// exception_test_check_02
  ...
  try {
      ...
      throw std::runtime_error("Caught unexpected exception in exception_test_check_02");
    }
  }
...
// exception_subclass_03
  {
    try {
      ...
      throw std::runtime_error("Caught unexpected exception in exception_subclass_03");
    }
  }
...
// nesting
  {
    ...
    std::equal(wanted_categories.begin(), wanted_categories.end(), e1.history().begin(),
               ensure_equal_to);
